...
with Namespace('tracks'):
    ...
    def turn_segment(radius, turn_degrees, block_type='ice', thickness=5, step_size=0.5, spiral_turn_degrees=0):
        ...
        with Fun() as f:
            ...
            with SelfEntity(scores='{i=0}').at_parent():
                next_chain()
            if spiral_turn_degrees:
                with SelfEntity(scores=f'{{i={int(num_steps) // 2}..}}').at(Pos.angular(forward=step_size), rot=Rot(relative=True, yaw=turn_sgn*degrees_per_step + spiral_turn_degrees)):
                    f()
                with SelfEntity(scores=f'{{i=1..{int(num_steps) // 2 - 1}}}').at(Pos.angular(forward=step_size), rot=Rot(relative=True, yaw=turn_sgn*degrees_per_step)):
                    f()
            else:
                with SelfEntity(scores='{i=1..}').at(Pos.angular(forward=step_size), rot=Rot(relative=True, yaw=turn_sgn*degrees_per_step)):
                    f()
    ...
    def straight_segment(length, block_type='ice', thickness=5, step_size=0.5):
        ...
        with Fun() as f:
            perpendiculars(block_type, thickness)
            Statement('scoreboard players remove @s i 1')
            with SelfEntity(scores='{i=0}').at_parent(): # TODO entity conditionals
                next_chain()
            with SelfEntity(scores='{i=1..}').at(Pos.angular(forward=step_size)):
                f()
        Statement(f'scoreboard players set @s i {int(num_steps) + 1}')
        f()
    ...
    def first_track():
        ...
        with SelfEntity().at(Pos.relative(y=-1), rot=Rot(pitch=0)):
            track_base(edge_block, 6)
            track_base(ice_block, 5)
        with SelfEntity().at(Pos.relative(y=-2), rot=Rot(pitch=0)):
            track_base(edge_block, 7)
        with SelfEntity().at(Pos.relative(y=-3), rot=Rot(pitch=0)):
            track_base(low_block, 4)

    @public
    def the_circle():
        with SelfEntity().at(Pos.angular(left=50), rot=Rot(pitch=0)):
            turn_segment(50, 360, 'packed_ice', 7)
        with SelfEntity().at(Pos.angular(left=46), rot=Rot(pitch=0)):
            turn_segment(46, 360, 'blue_ice', 4)


    @public
    def double_double_cake():
        with SelfEntity().at(Pos.relative(y=50)):
            with SelfEntity().at(Pos.angular(left=20), rot=Rot(pitch=0)):
                turn_segment(20, 360, 'packed_ice', 5)
            with SelfEntity().at(Pos.angular(left=16), rot=Rot(pitch=0)):
                turn_segment(16, 360, 'blue_ice', 4)
        
        with SelfEntity().at(Pos.relative(y=30)):
            with SelfEntity().at(Pos.angular(left=30), rot=Rot(pitch=-3)):
                turn_segment(30, 360, 'blue_ice', 3)

        with SelfEntity().at(Pos.relative(y=10)):
            with SelfEntity().at(Pos.angular(left=40), rot=Rot(pitch=-1)):
                turn_segment(40, 360, 'packed_ice', 7)
        
        with SelfEntity().at(Pos.relative(y=-10)):
            Statement('fill ~-42 ~ ~-42 ~42 ~ ~42 white_concrete')
            with SelfEntity().at(Pos.angular(left=52), rot=Rot(pitch=0)):
    ...
    def triple_helix():
        ...
        DEG = 3600//4
        with SelfEntity().at(Pos.relative(y=-10)):
            with SelfEntity().at(Pos.angular(left=R), rot=Rot(pitch=-7)):
                turn_segment(R, DEG + 2*360//3, 'packed_ice', 3)
        with SelfEntity().at(Pos.relative(y=-5)):
            with SelfEntity().at(Pos.angular(left=R), rot=Rot(pitch=-7)):
                turn_segment(R, DEG + 360//3, 'blue_ice', 3)
        with SelfEntity().at(Pos.relative(y=0)):
            with SelfEntity().at(Pos.angular(left=R), rot=Rot(pitch=-7)):
                turn_segment(R, DEG, 'ice', 3)
        with SelfEntity().at(Pos.relative(y=0)):
            with SelfEntity().at(Pos.angular(left=10), rot=Rot(pitch=0)):
