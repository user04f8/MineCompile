...
with Namespace('fun_test'):
    ...
    @fun
    def tnt_line():
        with SelfEntity().at(Pos.angular(forward=8)):
            line(
    ...
    def guide_line(step_dist_mult):
        with Summon('marker'):
            with SelfEntity().at(Pos.angular(forward=1, left=-.3)):
                line(
                    lambda: (
    ...
    def tick():
        with Entities(tag='laser_cannon'):
            ...
            with SelfEntity(scores=dict(charge='170..')):
        ...
        with Entities(tag='laser_cannon_sub') as e:
            Statement(f'scoreboard players add @s charge 1')
            with SelfEntity(scores=dict(charge='..5')):
                guide_line(6)
            with SelfEntity(scores='{charge=6..9}'):
                guide_line(7)
            with SelfEntity(scores='{charge=10..13}'):
                guide_line(8)
            with SelfEntity(scores='{charge=14..}'):
                tnt_laser()
                e.kill()
